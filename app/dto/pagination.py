...
class BaseRequest(BaseModel):
    ...
    def build_order_by(
        ...
        for raw_field in map(str.strip, self.order_by.split(",")):
            ...
            if desc_order:
                # При сортировке по убыванию NULL значения должны быть в конце
                clauses.append(column.desc().nulls_last())
            else:
                # При сортировке по возрастанию NULL значения должны быть в начале
                clauses.append(column.asc().nulls_first())
