...
from app.utils.types import JoinConfig
...
class BaseRepository(Generic[EntityType]):
    ...
    async def bulk_update(
        ...
        await self.session.execute(update(self.entity), updates)
    ...
    async def bulk_delete(
        ...
        # Обрабатываем большие списки по чанкам для избежания проблем с SQL
        chunk_size = 1000
        all_results = []

        for i in range(0, len(ids), chunk_size):
            chunk = ids[i : i + chunk_size]
            query = delete(self.entity).where(self.entity.id.in_(chunk))
            if returning:
        ...
        return all_results if returning else None
    ...
    async def soft_bulk_delete(
        ...
        for i in range(0, len(ids), chunk_size):
            ...
            query = (
                update(self.entity)
                .where(self.entity.id.in_(chunk))
                .values(deleted_at=datetime.now(timezone.utc))
            )
            if returning:
                query = query.returning(*returning)
